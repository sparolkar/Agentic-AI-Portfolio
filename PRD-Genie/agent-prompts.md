...
**Model:** gpt-4o-mini · **Temperature:** 0 · **Input:** raw text from the file node
...
INPUT: The raw text of one meeting transcript, product brief, or set of notes.
Quality varies: some inputs are detailed, some are vague, some contain
contradictions, some contain almost nothing.
...

{
  ...
  "not_discussed": ["<PRD topics with zero coverage in this input>"],
...
1) EVERY item in stated_requirements, ambiguous_items, contradictions,
   stakeholders, constraints and dependencies MUST include a "quote" field
   containing text copied verbatim from the input. If you cannot copy
